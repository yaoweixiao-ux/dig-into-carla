...
"""
"""
解析命令行 → 选择运行模式（普通场景 / OpenSCENARIO / OSC2 / Route）。
准备 CARLA Client / World、Traffic Manager、（可选）Agent。
加载场景并执行、记录结果、清理资源。
"""
...
class ScenarioRunner(object):
    ...
    def __init__(self, args):
        ...
        self._shutdown_requested = False #_shutdown_requested（信号中断标志)
    ...
    # 可选将每个 criterion 的 JSON 化字段汇总到文件
